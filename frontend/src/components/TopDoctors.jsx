...
const TopDoctors = () => {
  ...
  return doctors.length !== 0 ? (
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      ...
      <div className=" w-full flex flex-wrap justify-center gap-4 pt-5 gap-y-6 px-3 sm:px-0">
        {doctors.slice(0, 10).map((doctor, index) => (
          <div
            ...
            key={index}
          >
            <img className="bg-blue-50 w-60 md:w-52" src={doctor.image} alt={doctor.name} />
            <div className="p-4">
      ...
      <button onClick={()=>{navigate("/doctors"); scrollTo(0,0)}} className="bg-blue-50 text-gray-600 px-12 rounded-full py-3 mt-10 ">More</button>
    </div>
  ):(
    <Spiner text={"Loading Top Doctors..."}/>
  );
};
